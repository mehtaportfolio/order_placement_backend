...
export function buildPositionSyncPlan(formatted, existingToday, options = {}) {
    ...
    // CRITICAL FIX: Consolidate existing records with symbol variations
    // When DB has "ALPHA" and "ALPHA-EQ", keep only the normalized "ALPHA"
    // This prevents loss of duplicate detection when Map keys are overwritten
    const existingMap = new Map();
    (existingToday || []).forEach((row) => {
        const normalizedSymbol = normalizeSymbol(row.symbol);
        const existing = existingMap.get(normalizedSymbol);
        
        if (!existing) {
            // First record with this normalized symbol - store with normalized symbol name
            existingMap.set(normalizedSymbol, {
                ...row,
                symbol: normalizedSymbol  // Store normalized symbol
            });
        } else {
            // Multiple DB records with same normalized symbol found
            // This is a duplicate - keep the first one (or could merge quantities)
            // For now, we keep the first occurrence and log the duplicate
            console.log(`[AngelOneService] Duplicate symbol variant detected: "${row.symbol}" normalizes to "${normalizedSymbol}" (existing: "${existing.symbol}")`);
        }
    });
...
export async function fetchTodayBuyTrades(options = {}) {
    ...
    try {
        ...
        // Include ID for proper duplicate detection and upsert operations
        const { data: existingToday } = await fetchAllRows(supabase, 'equity_positions', {
            select: 'id, symbol, quantity, average_price, broker, account_id, exchange, product, pnl',
            filters: [
